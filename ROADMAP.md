...
- 🟢 **[SDD 28 — Métricas de uso LLM por usuario + GPU](docs/sdd-llm-usage-metrics.md)**: **app HECHA** — `llm_chat` pasa `user` (`player:`/`npc:`) → `end_user` en LiteLLM (atribución por jugador/backend) + métrica propia `game_llm_calls_total{kind}`. Pendiente INFRA: DCGM-exporter (% cómputo) + dashboards billing/GPU.
- 🟢 **[SDD 29 — Inteligencia estratégica de NPCs](docs/sdd-npc-strategic-intelligence.md)**: **v1 hecho** — cerebro de 2 capas (estratégico periódico lee scoreboard+recursos y fija postura; táctico la ejecuta, también en reglas). Medible por NPC (SDD 28), fallback a reglas. **Reflexión post-batalla HECHA** (determinista: ajusta postura por resultado).
...
- 🟢 **[SDD 34 — Estrategia de combate (fórmula + calculadora + IA)](docs/sdd-combat-strategy.md)**: **COMPLETO** — fórmula + matriz documentadas; `combat_calc.py` + `POST /combat/simulate` y `/combat/plan` (estima defensa desde tu intel); **calculadora web 🧮** con sliders en vivo; asistente IA aterrizado (grounding `mech_combat_planning` → usa las herramientas, no alucina).
