...
def test_swarm_of_sonics_saturates_interception():
    # capacidad 3 (= 3 torretas × intercept_power, normalizado) frena 3 sónicos (cost 1 c/u);
    # los 2 que sobran impactan → daño 2×60.
    r = simulate_strike({"sonic_missile": 5}, intercept_capacity=3)
...
def test_nuclear_is_nearly_unstoppable():
    # un nuclear cuesta 8 de capacidad: con capacidad 5 no se frena → impacta (daño 600 + área).
    r = simulate_strike({"nuclear_missile": 1}, intercept_capacity=5)
...
def test_cheap_missiles_are_intercepted_first():
    # la capacidad se gasta primero en los baratos (sónico cost 1), luego el crucero (cost 3).
    r = simulate_strike({"sonic_missile": 2, "cruise_missile": 1}, intercept_capacity=2)
    assert r.intercepted == {"sonic_missile": 2}      # los 2 baratos
...
def test_full_interception_no_damage():
    r = simulate_strike({"sonic_missile": 3}, intercept_capacity=10)
